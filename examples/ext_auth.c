...
static char *_plain_mechanism_authdata(xmpp_auth_t *auth)
{
    ...
    str = malloc(node_len + pass_len + 2);
    str[0] = '\0';
    strncpy(str + 1, node, node_len);
    str[node_len + 1] = '\0';
    strncpy(str + node_len + 2, auth->pass, pass_len);
    b64 = xmpp_base64_encode(auth->ctx, str, node_len + pass_len + 2);
