...
#ifndef LIBSTROPHE_SHA_H__
#define LIBSTROPHE_SHA_H__
...
#define STORE32H(x, y)        \
  do { uint32_t __t = (x); memcpy(y, &__t, 4); } while(0)

#define LOAD32H(x, y)         \
  do { memcpy(&(x), y, 4); x &= 0xFFFFFFFF; } while(0)

#define STORE64H(x, y)        \
  do { uint64_t __t = (x); memcpy(y, &__t, 8); } while(0)
